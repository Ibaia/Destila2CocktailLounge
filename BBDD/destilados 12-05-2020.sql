...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 12-05-2020 a las 10:58:33
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spGetIdByPackName` (IN `pName` VARCHAR(100))  NO SQL
SELECT * FROM packs
WHERE packs.nombrePack=pName$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spInsertReserva` (IN `pFecha` DATE, IN `pIdUsu` INT(11), IN `pPack` INT(11))  NO SQL
INSERT INTO `reservas` (`fecha`, `idUsuario`, `idPack`) VALUES (pFecha, pIdUsu, pPack)$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spReservasUsu` (IN `pUsu` INT)  NO SQL
SELECT reservas.idReserva, reservas.fecha, reservas.idUsuario,reservas.idPack,packs.nombrePack
FROM reservas
JOIN packs ON reservas.idPack=packs.idPack
WHERE reservas.idUsuario=pUsu$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spUpdateReserva` (IN `pIdReserva` INT, IN `pFecha` DATE, IN `pPack` INT)  NO SQL
UPDATE `reservas` SET
reservas.fecha=pFecha, reservas.idPack=pPack
WHERE reservas.idReserva=pIdReserva$$
